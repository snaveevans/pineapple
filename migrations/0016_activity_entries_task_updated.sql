-- Widen activity_entries.type to allow 'task_updated' (maintenance-task S2 /
-- activity-history S3, issue #180). SQLite has no ALTER TABLE for CHECK
-- constraints, so this rebuilds the table: create it with the widened CHECK,
-- copy existing rows across unchanged, then swap it in. No column, row, or
-- index is dropped or narrowed — old code inserting any of the five original
-- type values keeps working unmodified.

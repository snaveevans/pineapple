...
- Threads where the **last reply is yours** — you already answered; a reply is not a new
  request.
- Your **own** comments, review summaries, and status tables echoed back to you.
...
Bot comments (Dependabot, coverage bots, prior Claude comments) go through the same
triage — a bot flagging a genuine failure is a real finding, and a bot's style nit is
still a nit.
...
Replies go **in the thread** that raised the point, not as new top-level comments — a
reviewer scanning their notifications should see the answer attached to their question.
Use the reply-to-review-comment path (`gh api`, `add_reply_to_pull_request_comment`, or
equivalent), then resolve the thread once the fix is pushed.
...
Out-of-scope findings are real work — they just are not _this branch's_ work. Losing
them is how good review feedback evaporates.

Propose each one to the user with a title and a two-line body before creating anything.
Creating issues is outward-facing and permanent; a proposal costs nothing. Once
confirmed, create them and reference the numbers in your replies.

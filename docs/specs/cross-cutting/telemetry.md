...
**Domain event data point — `TeamCreated`** (dataset: `pineapple_team_domain_events`, index:
`owner_id`):
...
| `blobs[3]`   | `owner_id`        | Owner UUID                                                                 |
| `blobs[4]`   | `actor_id`        | UUID of the user who created the team — currently always equals `owner_id` |

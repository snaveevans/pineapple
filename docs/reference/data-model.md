...
| `MaintenanceTaskRescheduled` | a task's current due date is moved without logging maintenance | event id, task/asset/owner/actor, asset snapshot, title, resulting **`nextDue`** (the override), resulting `taskRevision`, and History `task_rescheduled` conclusion; published only when the target differs from the current effective `nextDue` |
...
the resulting task revision but is not an Activity History event. `MaintenanceTaskRescheduled`
carries the resulting task revision with its `task_rescheduled` History conclusion.
...
| `MaintenanceTask`                | `maintenance_tasks`                          | Stores mutable `last_completed_date`/`next_due` plus immutable `schedule_seed_date` and nullable `initial_last_completed_date` used to rebuild a task after linked-record corrections; nullable `next_due_override` holds the current one-cycle reschedule target (cleared by interval edit or a successful task advance, never inferred from a matching `next_due`) |
